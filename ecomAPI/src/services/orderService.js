        ...
        try {
            if (!data.addressUserId || !data.typeShipId) {
                ...
                for (let i = 0; i < data.arrDataShopCart.length; i++) {
                    ...
                    productQuantity.push({ productId: data.arrDataShopCart[i].productId, total: quantity })
                }
                for (let i = 0; i < productQuantity.length; i++) {
                    if (productQuantity[i].total < data.arrDataShopCart[i].quantity) {
                        resolve({
                            errCode: 1,
                            errMessage: `Sản phẩm ${i + 1} chỉ còn ${productQuantity[i].total} sản phẩm`
                        })
                        return
                    }
                }
            ...
            if (!id) {
                ...
                for (let i = 0; i < orderDetail.length; i++) {
                    ...
                    for (let j = 0; j < orderDetail[i].productImage.length; j++) {
                        orderDetail[i].productImage[j].image = Buffer.from(orderDetail[i].productImage[j].image, 'base64').toString('binary')
                    }
                }
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < data.result.length; i++) {
                ...
                productQuantity.push({ productId: data.result[i].productId, total: quantity })
            }
            for (let i = 0; i < productQuantity.length; i++) {
                if (productQuantity[i].total < data.result[i].quantity) {
                    resolve({
                        errCode: 1,
                        errMessage: `Sản phẩm ${i + 1} chỉ còn ${productQuantity[i].total} sản phẩm`
                    })
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < req.body?.arrDataShopCart.length; i++) {
                ...
                productQuantity.push({ productId: req.body?.arrDataShopCart[i].productId, total: quantity })
            }
            for (let i = 0; i < productQuantity.length; i++) {
                if (productQuantity[i].total < req.body?.arrDataShopCart[i].quantity) {
                    resolve({
                        errCode: 1,
                        errMessage: `Sản phẩm ${i + 1} chỉ còn ${productQuantity[i].total} sản phẩm`
                    })
